...
export default function NewCategoryPage() {
  ...
  // Buscar categorias principais para o select de subcategoria
  useEffect(() => {
    const fetchParentCategories = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setParentCategories(data.filter((c: Category) => !c.parentId));
        }
      } catch (err) {
  ...
  const renderIcon = (iconId: string) => {
    const icons: Record<string, JSX.Element> = {
      ...
      "shopping-cart": <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>,
      utensils: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>,
    };
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <header className="relative z-10 w-full p-6 flex justify-between items-center backdrop-blur-sm bg-slate-950/50 border-b border-emerald-500/20">
        <div className="flex items-center gap-3">
          <Link href="/dashboard/categorias" className="flex items-center gap-3">
      ...
      <main className="relative z-10 p-8">
        <div className="max-w-2xl mx-auto">
          {/* Título */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Nova Categoria</h1>
            <p className="text-slate-400">Cadastre uma nova categoria de receita ou despesa</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-slate-900/50 backdrop-blur-md rounded-2xl border border-emerald-500/20 p-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {CATEGORY_TYPES.map((type) => (
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full ${type.value === "INCOME" ? "bg-emerald-400" : "bg-red-400"}`}></span>
                    <span className={`font-medium ${formData.type === type.value ? type.color : "text-slate-400"}`}>
                      {type.label}
                    </span>
                  </button>
                ))}
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Deixe em branco para criar uma categoria principal, ou selecione uma categoria existente do mesmo tipo para criar uma subcategoria
              </p>
            </div>
          ...
          {/* Dicas */}
          <div className="mt-6 p-6 bg-slate-900/30 rounded-2xl border border-slate-800">
            <h3 className="text-sm font-semibold text-slate-300 mb-3">💡 Dicas:</h3>
            <ul className="space-y-2 text-sm text-slate-400">
              <li className="flex items-start gap-2">
                <span className="text-emerald-400 mt-1">•</span>
                <span>Categorias de <strong>Receita</strong> são para entradas de dinheiro (salário, investimentos, etc.)</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-red-400 mt-1">•</span>
                <span>Categorias de <strong>Despesa</strong> são para saídas de dinheiro (alimentação, transporte, etc.)</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-400 mt-1">•</span>
                <span>Você pode criar subcategorias para organizar melhor (ex: Alimentação → Restaurante, Mercado)</span>
              </li>
            </ul>
          </div>
        </div>
