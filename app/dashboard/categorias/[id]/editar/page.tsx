...
export default function EditCategoryPage() {
    ...
    const fetchParentCategories = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setParentCategories(data.filter((c: Category) => !c.parentId && c.id !== categoryId));
        }
      } catch (err) {
  ...
  const handleArchive = async () => {
    if (!confirm(`Tem certeza que deseja arquivar "${formData.name}"? Esta ação não pode ser desfeita facilmente.`)) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.transactionCount > 0) {
          alert(`Não é possível arquivar esta categoria porque possui ${data.transactionCount} transações vinculadas.`);
        } else {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <Link href="/dashboard/categorias" className="text-emerald-400 hover:underline">
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="relative z-10 p-8">
        <div className="max-w-2xl mx-auto">
          {/* Título */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Editar Categoria</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-slate-900/50 backdrop-blur-md rounded-2xl border border-emerald-500/20 p-8">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Deixe em branco para categoria principal, ou selecione uma categoria do mesmo tipo para criar subcategoria
              </p>
            </div>
          ...
          <div className="mt-6 p-6 bg-red-500/10 rounded-2xl border border-red-500/20">
            ...
            <p className="text-slate-400 text-sm mb-4">
              Arquivar esta categoria irá ocultá-la da lista principal. Não é possível arquivar categorias com transações vinculadas.
            </p>
            <button
