...
export default function AccountsPage() {
  ...
  const fetchAccounts = useCallback(async () => {
    ...
    try {
      ...
      const accountsData = Array.isArray(result) ? result : (result.accounts || []);
            ...
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400">Gerencie suas contas bancárias e cartões</p>
                </div>
              </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-300 mt-1">
                  Contas arquivadas não aparecem no dashboard. Você pode restaurá-las.
                </p>
              </div>
            </div>
          )}
          ...
          {!loading && !error && accounts.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {accounts
                .filter((account) => (showArchived ? !account.isActive : account.isActive))
                .map((account) => (
                    ...
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-500">
                          <span>Criada em {new Date(account.createdAt).toLocaleDateString("pt-BR")}</span>
                        </div>
                      </div>
                    </TooltipTrigger>
                  </Tooltip>
            ...
            <div className="mt-8 p-4 bg-slate-900/50 backdrop-blur-md rounded-xl border border-slate-800">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="text-sm text-slate-400">
                  <span className="text-white font-semibold">{accounts.filter((a) => a.isActive).length}</span> contas ativas
                </div>
                <div className="text-lg font-bold text-white">
                  Total:{" "}
                  <span className={accounts.reduce((sum, a) => sum + a.balance, 0) >= 0 ? "text-emerald-400" : "text-red-400"}>
