...
export async function PUT(
  ...
  try {
    ...
    if (parentId && parentId !== existingCategory.parentId) {
      ...
      if (!parentCategory) {
        return NextResponse.json({ error: "Categoria pai inválida" }, { status: 400 });
      }
    }
    ...
    return NextResponse.json(
      { message: "Categoria atualizada com sucesso", category: updatedCategory },
      { status: 200 }
    );
  } catch (error: any) {
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Categoria arquivada com sucesso", category: archivedCategory },
      { status: 200 }
    );
  } catch (error: any) {
