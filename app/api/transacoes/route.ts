...
export async function GET(request: NextRequest) {
  try {
    ...
    const whereClause: any = {
      userId: user.id,
      isDeleted: showArchived, // Se true, busca na lixeira, se false, busca ativas
    };
    ...
    // 6. Retornar Resposta — Mapear isDeleted → isArchived para compatibilidade com o frontend
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Campos obrigatórios faltando (description, amount, type, accountId, categoryId, occurrenceDate)",
        },
        { status: 400 },
      );
    }
    ...
    if (typeof amount !== "number" || amount <= 0) {
      return NextResponse.json(
        { error: "Valor deve ser um número positivo em centavos" },
        { status: 400 },
      );
    }
