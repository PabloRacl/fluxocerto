...
export function GraficoDespesasPorCategoria({
  ...
  useEffect(() => {
    const fetchExpensesData = async () => {
      try {
        ...
        if (period === "month") {
          // Início do mês em UTC (00:00:00Z) para bater com o resumo rápido (CartoesResumo)
  ...
  // ============================================
  if (loading) {
    return <CarregamentoNeural message="Categorizando Fluxo de Gastos..." variant="card" />;
  }
    ...
    <div data-tour="chart-categorias" className="relative bg-slate-950/40 backdrop-blur-2xl rounded-3xl border border-slate-800/50 p-6 shadow-2xl h-full flex flex-col group overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-6 relative z-10">
        ...
        <div className="flex items-center gap-2 transition-all">
          ...
          <div className="flex items-center gap-1 bg-slate-900/80 backdrop-blur-md rounded-xl p-1.5 border border-white/5 hidden sm:flex">
            <button
      ...
      >
        ...
        <div className="relative w-[220px] h-[220px] flex-shrink-0 flex items-center justify-center">
            ...
            <ResponsiveContainer width="100%" height="100%">
                    ...
                    {/* Tooltip removido por ser redundante com o Neuro Core e causar glitch visual */}
                </PieChart>
            </ResponsiveContainer>
        </div>
      ...
      {/* Modal Ver Mais Categorias (usando ModalAnimado existente) */}
      <ModalAnimado
        isOpen={showAllModal}
      ...
      >
        <div className="flex flex-col h-[75vh]">
            <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar">
                <div className="space-y-4">
                    {data.map((item, index) => (
                        ...
                        >
                            {/* Header da Categoria */}
                            <div className="flex items-center justify-between px-4 py-3 bg-slate-800/40 border-b border-slate-800/60">
                            ...
                            <div className="divide-y divide-slate-800/40">
                                {item.transactions && item.transactions.length > 0 ? (
                                    item.transactions.map((t: any) => (
                                        <div
                                            key={t.id}
                                            className="flex items-center justify-between py-2.5 px-4 hover:bg-slate-800/40 transition-colors"
                                        >
                                            ...
                                            <span className="text-xs font-black text-slate-200 ml-4">
            ...
            <div className="px-6 py-4 bg-slate-900 border-t border-slate-800 flex justify-between items-center shrink-0">
                ...
                <span className="text-xl font-black text-white" style={{ textShadow: "0 0 15px rgba(239,68,68,0.5)" }}>
                    {formatCurrency(totalExpenses)}
                </span>
            </div>
        </div>
