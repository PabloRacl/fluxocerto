...
export default function ModalEditarDivida({
  isOpen,
  onClose,
  debt,
}: EditDebtModalProps) {
  ...
  useEffect(() => {
    if (debt && isOpen) {
      setForm({
        ...
        accountId: debt.accountId || debt.account?.id || "",
        categoryId: debt.categoryId || debt.category?.id || "",
    ...
    try {
      ...
      const res = await fetch(`/api/dividas/${debt.id}`, {
        ...
        body: JSON.stringify({
          ...
          totalAmount: Math.round(parseFloat(String(form.totalAmount).replace(',', '.')) * 100),
          installmentValue: Math.round(parseFloat(String(form.installmentValue).replace(',', '.')) * 100),
          installmentTotal: parseInt(String(form.installmentTotal)),
          installmentPaid: parseInt(String(form.installmentPaid)),
          interestRate: form.interestRate ? parseFloat(String(form.interestRate).replace(',', '.')) : null,
    ...
    >
      <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="flex gap-4 pt-4 sticky bottom-0 bg-slate-900/80 backdrop-blur-sm -mx-2 px-2">
          ...
          >
            {loading ? (
              <CarregamentoNeural message="Sincronizando..." variant="inline" />
            ) : (
              <>
